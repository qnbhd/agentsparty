# Boundary (/docs/agentsparty/protocol/Boundary)
...
def defining(self, body: Fragment[SessionType] | SessionType) -> SessionType:
    ...
    Args:
        body: An open fragment or closed session protocol whose every
...
An open fragment or closed session protocol whose every
``msg``/``alt`` prefix will be reclassified.
