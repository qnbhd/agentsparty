# protocol (/docs/agentsparty/protocol/index)
...
<Card title={"Boundary"} href={"/docs/agentsparty/protocol/Boundary"} />
...
A single-subject session type (typically ``project_onto(H, \{r\})``).
...
def as_global(node: SessionType) -> GlobalType:
    ...
    Args:
        node: A well-formed session type.
...
Reject open, unguarded, or ill-partitioned session types.
...
def assert_wellformed(node: SessionType) -> None:
    ...
    Args:
        node: The session type to validate.
...
<PyFunction name={"associate"} type={"(declared, proto, role) -> EndpointType"}>
...
<PyParameter name={"declared"} type={"EndpointType | None"} value={undefined}>

The endpoint type the participant claims, or ``None`` to take
the projection of *proto* on *role*.

</PyParameter>
<PyParameter name={"proto"} type={"SessionType"} value={undefined}>

The choreography the participant is bound to.

</PyParameter>
<PyParameter name={"role"} type={"Role"} value={undefined}>

The role the participant plays.
...
def compose(contract: SessionType, components: Sequence[SessionType]) -> SessionType:
    ...
    Returns:
        The composed session type (global when *contract* and all interfaces absorb).
...
Boundaries that historically required a sealed session type
...
def ensure_session(node: SessionType | Fragment[SessionType]) -> SessionType:
    ...
    Args:
        node: A closed session type, or a still-open fragment of one.
...
A closed session type, or a still-open fragment of one.
...
A closed, guarded session type (typically a component).
...
state graph of a well-formed session type.
...
A closed, guarded session type (choreography or a component),
or a fragment that will be closed first.
...
The session type to project, or a fragment that will be closed.
...
<PyFunction name={"subtype"} type={"(sub, sup) -> bool"}>
...
[`Decision`](/docs/agentsparty/journal/types/Decision) records, and the only one that
...
<PyParameter name={"sub"} type={"EndpointType"} value={undefined}>

The endpoint type on offer.

</PyParameter>
<PyParameter name={"sup"} type={"EndpointType"} value={undefined}>

The endpoint type required.
...
<Card href={"/docs/agentsparty/protocol/render"} title={"render"} />
<Card href={"/docs/agentsparty/protocol/routine"} title={"routine"} />
<Card href={"/docs/agentsparty/protocol/graph"} title={"graph"} />
<Card href={"/docs/agentsparty/protocol/analysis"} title={"analysis"} />
<Card href={"/docs/agentsparty/protocol/language"} title={"language"} />
<Card href={"/docs/agentsparty/protocol/session"} title={"session"} />
