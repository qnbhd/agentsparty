...
`equal_session` holds against the combinator twin.

<ChoreographyEquivalence />

Both spellings build the identical tree, so everything downstream —
`render`, `project`, `Cast` — cannot tell which one produced it. Pick the
facade that reads better for the protocol at hand.
