...
either becomes a local action for a role or disappears from that role's
endpoint entirely:

Spelled out, the fan-out is three columns beside the global messages: each
global line and its local counterparts sit on one row.
...
For a alt, the chooser gets an internal `select`, the informed peer gets an
