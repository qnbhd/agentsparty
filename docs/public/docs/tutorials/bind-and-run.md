...
List = pa.Text('List')
Listing = pa.Text.many()('Listing')
Read = pa.Text('Read')
Source = pa.Text('Source')
Write = pa.record('Write', path=str, content=str)('Write')
Saved = pa.Text('Saved')
Noted = pa.Nothing('Noted')
Ready = pa.Nothing('Ready')
Done = pa.Nothing('Done')
Idle = pa.Nothing('Idle')


async def ack(_empty: None) -> pa.Choice:
...
def workspace_tools(root: Path) -> list[Any]:
    ...
    return [
        ...
        pa.tool_for(Ready, ack),
        pa.tool_for(Write, write),
        pa.tool_for(Done, ack),
        pa.tool_for(Idle, ack),
    ]
```
...
trace = (
    ...
    .play(
        shipped['Client'],
        pa.human(pa.CliHumanIo()),
    )
    .play(
        shipped['Planner'],
        pa.agent(model, shipped['PLANNER_BRIEF'], repair=pa.Repair(attempts=2)),
    )
    .play(
        shipped['Coder'],
        pa.agent(model, shipped['CODER_BRIEF'], repair=pa.Repair(attempts=2)),
    )
    .play(
        shipped['Reviewer'],
        pa.agent(model, shipped['REVIEWER_BRIEF'], repair=pa.Repair(attempts=2)),
    )
    .play(
        shipped['Workspace'],
        pa.service(*shipped['workspace_tools'](root)),
    )
    .run_sync(allowance=pa.Allowance(unfoldings=12))
)
