...
const jsxComponentStart = /^\s*<[A-Z][A-Za-z0-9.]*(?:\s|$)/;
const jsxComponentEnd = /\/?>\s*$/;
...
for (const file of await filesUnder(root)) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (fenced) continue;
    if (multilineJsx) {
      if (jsxComponentEnd.test(line)) multilineJsx = false;
      continue;
    }
    if (jsxComponent.test(line)) continue;
    if (jsxComponentStart.test(line)) {
      multilineJsx = !jsxComponentEnd.test(line);
      continue;
    }
    if (/(?<!\\)[{}]|(?<![\\\w])</.test(line)) {
