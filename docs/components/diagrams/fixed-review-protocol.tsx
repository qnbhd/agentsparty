...
function Message({
  ...
  const marker = `pdsArrow${step.from}`;
...
function ProtocolSvg({ progress }: { progress: number }) {
  ...
  return (
    <svg className={styles.svg} aria-label="Animated sequence diagram of the review protocol" viewBox="0 0 720 380" xmlns="http://www.w3.org/2000/svg">
      <defs>
        {/* One arrowhead per role, filled from the same token as its lifeline
          * and label, so the marker follows the theme like everything else. */}
        {Object.entries(COLORS).map(([role, color]) => (
          <marker key={role} id={`pdsArrow${role}`} markerHeight="8" markerWidth="8" orient="auto" refX="6" refY="3">
