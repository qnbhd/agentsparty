...
Intake, Classifier, KB, Responder, Approver = pa.roles(
    'Intake',
    'Classifier',
    'KB',
    'Responder',
...
Ticket = pa.record('Ticket', text=str, customer_tier=str)(
    'Ticket',
    'Customer message and support tier.',
)
Search = pa.Text('Search', 'A short query for the knowledge base.')
Hits = pa.Text.many()('Hits', 'Matching knowledge-base articles.')

Severity = pa.Integer.where('severity_1_to_5', lambda n: 1 <= n <= 5)
...
Spam = pa.Nothing('Spam', 'Not a real support request.')

OnBilling = pa.Nothing('OnBilling', 'This ticket is a billing case.')
OnTechnical = pa.Nothing('OnTechnical', 'This ticket is a technical case.')
OnRefund = pa.Nothing('OnRefund', 'This ticket is a refund case.')

Draft = pa.record('Draft', subject=str, body=str)(
    'Draft',
    'A reply the customer can receive.',
)
Ship = pa.Nothing('Ship', 'Send the draft to the customer.')
Escalate = pa.Text('Escalate', 'Why a human should take this ticket.')
Shipped = pa.Nothing('Shipped', 'The draft was sent.')
Escalated = pa.Nothing('Escalated', 'A human will take this ticket.')
Close = pa.Nothing('Close', 'Close the ticket as spam.')
...
def _handled(c: pa.Chor, notice: Case) -> None:
...
def build_participants(
    ...
    intake = pa.Human(
        Intake,
        protocol,
        pa.ScriptedHumanIo([pa.says(Ticket, ticket), pa.says(Ticket, ticket)]),
    )
    classifier = pa.Agent(
        model,
        Classifier,
    ...
    return [
        ...
        classifier,
        responder,
        pa.Human(Approver, protocol, approver_io),
        pa.Toolbox(KB, protocol, [pa.tool_for(Search, search)]),
    ]
