...
Editor, Bard, Critic, Meter = ap.roles('Editor', 'Bard', 'Critic', 'Meter')

POST = ap.Text.where('at most 280 characters', lambda s: len(s) <= 280)
...
Length = ap.Integer('Length', 'Character count of the draft.')
Closed = ap.Text('Closed', CLOSED)
Measure = ap.Text('Measure', 'The draft to measure.')
Done = ap.Nothing('Done', 'No further draft is coming.')
Again = ap.Nothing('Again', 'Another draft is coming.')
Topic = ap.Text('Topic', 'What the post is about.')
Post = POST('Post', 'A Shakespearean post within the limit.')
Final = POST('Final', 'The accepted post.')
Abandoned = ap.Text('Abandoned', 'Why nothing was produced.')
Retrying = ap.Text('Retrying', PROGRESS)
Ok = ap.Nothing('Ok', 'Accept the post.')
GiveUp = ap.Text('Give up', 'Why this post will not converge.')
Revise = ap.Text('Revise', 'One critique, at most twenty-five words.')
...
def _meter_tools() -> list[Any]:
    async def measure(draft: str) -> ap.Choice:
        return ap.reply(Length, len(draft))

    async def done(_empty: None) -> ap.Choice:
        return ap.reply(Closed, CLOSED)

    async def again(_empty: None) -> ap.Choice:
        return ap.reply(Closed, CLOSED)

    return [
        ap.tool_for(Measure, measure),
        ap.tool_for(Done, done),
        ap.tool_for(Again, again),
    ]
...
def _critic_decide(max_attempts: int, *, always_revise: bool = False):
    def decide(view: ap.View) -> ap.Choice:
        ...
        if always_revise:
            return ap.says(Revise, 'tighten the meter')
        if max_attempts == 2 and attempts >= 2:
            return ap.says(Ok, None)
        if attempts >= max_attempts:
            return ap.says(GiveUp, 'three drafts still miss the mark')
        return ap.says(Revise, 'tighten the meter')
...
def build(
    *,
...
) -> tuple[SessionType, list[ap.Participant]]:
    # give-up: Critic yields on third Post; ok: accepts on second; limit: always Revise.
    bard = ap.Agent(
        ap.ScriptedLanguageModel(_bard_script(path)),
        Bard,
    ...
    editor = ap.Human(
        Editor,
        protocol,
        ap.ScriptedHumanIo([ap.says(Topic, 'ambition and rest')]),
    )
    return protocol, [editor, bard, critic, ap.Toolbox(Meter, protocol, _meter_tools())]
...
def _run(
    ...
    *,
    ...
    allowance = ap.Allowance(unfoldings=unfoldings) if unfoldings is not None else ap.Allowance()
    runtime = ap.AgentRuntime(protocol, participants, allowance=allowance)
    try:
        return runtime.run_sync()
    except ap.RecursionLimitError:
