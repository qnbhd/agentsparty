...
What you will see (exact):
  ...
  === second attempt ===
  Lead:task -> Analyst 'multiparty session protocols'
  Analyst:search -> Tools 'multiparty session protocols'
  Tools:hits -> Analyst [...]
...
Lead, Analyst, Tools = ap.roles('Lead', 'Analyst', 'Tools')
...
@ap.choreography
def build_protocol(c: ap.Chor) -> None:
...
async def _search(query: str) -> ap.Choice:
    """Answer a search out of a tiny in-memory index."""
    return ap.reply(Hits, ['Honda et al.', 'Bocchi et al.', 'timed MPST'])


def _analyst(answers: list[str]) -> ap.Agent:
    return ap.Agent(
...
def _fresh_lead() -> tuple[ap.Human, ap.ScriptedHumanIo]:
    """A delegating human with one scripted task, and the io that records it."""
    io = ap.ScriptedHumanIo([ap.says(Task, 'multiparty session protocols')])
    return ap.Human(Lead, protocol, io), io
...
def main() -> None:
    ...
    script = [
        json.dumps(
            {'alt': {'label': 'search', 'payload': 'multiparty session protocols'}},
        ),
        json.dumps({'alt': {'label': 'answer', 'payload': 'three sources found'}}),
    ]
    box = ap.Toolbox(Tools, protocol, [ap.tool_for(Search, _search)])

    report.note('allowance for two steps; protocol needs four', title='first attempt')
    lead, io = _fresh_lead()
    crashed = ap.MemoryTracer()
    try:
        ap.AgentRuntime(
            protocol,
            [lead, _analyst(list(script)), box],
            allowance=ap.Allowance(steps=2),
            tracer=crashed,
        ).run_sync()
    except ap.StepLimitError as error:
        report.note(
    ...
    retried = ap.MemoryTracer()
    trace = ap.AgentRuntime(
        protocol,
