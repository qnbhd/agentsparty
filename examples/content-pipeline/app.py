...
MODEL = 'gpt-5.6-luna'
TOKEN_LIMIT = 80_000
ALLOWANCE = ap.Allowance(steps=40, unfoldings=3)
...
class Studio:
    ...
    runtime: ap.AgentRuntime | None = None
    task: asyncio.Task[None] | None = None
    error: str | None = None
    revision: int = 0

    def model(self) -> ap.LanguageModel:
        ...
        model = ap.OpenAIModel(MODEL, self.client)
    ...
    async def _run(self) -> None:
        ...
        runtime = cast(ap.AgentRuntime, self.runtime)
        try:
            await runtime.run()
