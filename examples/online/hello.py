...
Note = ap.Text('Note', 'One short sentence.')
Ack = ap.Text('Ack', 'A one-word acknowledgement.')
...
def main() -> None:
    ...
    cast = (
        ap
        .Cast(protocol)
        .play(Writer, ap.agent(model, 'Send one short sentence as Note.'))
        .play(Reviewer, ap.agent(model, 'Reply Ack with one short word.'))
    )
