...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    worker = pa.Agent(model, Worker, 'Do the work in one short draft.', protocol)
    reviewer = pa.Agent(model, Reviewer, 'Accept or note one change.', protocol)
    client = pa.Human(
        Client,
        protocol,
        pa.ScriptedHumanIo([pa.says(Task, 'summarise the provider story')]),
    )
