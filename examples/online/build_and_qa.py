...
Product, Engineer, Reviewer, Chief = pa.roles('Product', 'Engineer', 'Reviewer', 'Chief')

CODE = pa.Text.where('a fenced code block', lambda s: s.count('```') >= 2)
...
Spec = pa.Text('Spec', 'What the program must do.')
Code = CODE('Code', 'The current implementation.')
Clean = CODE('Clean', 'Code in which no defect was found.')
Iterating = pa.Text('Iterating', ITERATING)
Release = CODE('Release', 'The released program.')
Again = pa.Nothing('Again', 'Another review is coming.')
Ship = pa.Nothing('Ship', 'Release the implementation.')
StandDown = pa.Nothing('Stand down', 'No more review.')
Defects = pa.Text('Defects', 'The defects found, one per line.')
Rework = pa.Text('Rework', REWORK)
...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    product = pa.Human(
        Product,
        protocol,
        pa.ScriptedHumanIo([pa.says(Spec, 'a function that adds two numbers')]),
    )
    return protocol, [
        product,
        pa.Agent(
