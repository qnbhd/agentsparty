...
Author, Fixer, Linter = pa.roles('Author', 'Fixer', 'Linter')

FINDINGS = pa.Text.many(1)


Clean = pa.Text('Clean', 'No violation found.')
Scan = pa.Text('Scan', 'The current document text.')
Document = pa.Text('Document', 'The markdown to clean up.')
Findings = FINDINGS('Findings', 'One line per rule violation.')
Progress = pa.Integer('Progress', 'How many findings remain.')
...
def _linter_tools() -> list[Any]:
    async def scan(document: str) -> pa.Choice:
        findings: list[RawValue] = list(_lint(document))
        if findings:
            return pa.reply(Findings, findings)
        return pa.reply(Clean, None)

    return [pa.tool_for(Scan, scan)]
...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    author = pa.Human(
        Author,
        protocol,
        pa.ScriptedHumanIo([pa.says(Document, DOC_V0)]),
    )
    participants: list[pa.Participant] = [
        author,
        fixer,
        pa.Toolbox(Linter, protocol, _linter_tools()),
    ]
