...
Traveller, Planner, Scout, Compiler, Concierge = pa.roles(
    'Traveller',
    'Planner',
    'Scout',
    'Compiler',
...
Preferences = pa.Text('Preferences', 'Dates, budget and what the traveller likes.')
Shortlist = pa.Text.many()('Shortlist', 'Candidate activities.')
Vetted = pa.Text.many()('Vetted', 'Activities worth including.')
ItineraryMsg = ITINERARY('Itinerary', 'The full itinerary, day by day.')
Departure = pa.Text('Departure', 'When and where to be, and nothing else.')
Go = pa.Nothing('Go', 'Accept the surprise as it stands.')
Confirmed = pa.Nothing('Confirmed', 'The traveller is going.')
Reveal = pa.Text('Reveal', 'Why the traveller wants the surprise spoiled.')
Spoiled = pa.Text('Spoiled', 'Why the surprise was dropped.')
...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    traveller = pa.Human(
        ...
        pa.ScriptedHumanIo(
            [
                pa.says(Preferences, 'July, mid budget, quiet coast walks'),
                pa.says(Go, None),
            ],
        ),
    )
