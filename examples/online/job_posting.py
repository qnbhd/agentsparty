...
Hiring, Researcher, Writer, Reviewer = pa.roles(
    'Hiring',
    'Researcher',
    'Writer',
    'Reviewer',
...
REQUIREMENTS = pa.json_model('Requirements', REQUIREMENTS_SCHEMA, parse_requirements).where(
    'no empty section',
    lambda r: all((r.skills, r.experience, r.qualities)),
)


Draft = pa.Text('Draft', DRAFT)
Industry = pa.Text('Industry', 'Market context the reviewer must weigh.')
Publish = pa.Text('Publish', POSTING)
Posting = pa.Text('Posting', POSTING)
RequirementsCase = REQUIREMENTS('Requirements', 'Skills, experience and qualities.')
Company = pa.Text('Company', 'Domain, description and hiring need.')
Culture = pa.Text('Culture', 'What the company is like to work at.')
Rewrite = pa.Text('Rewrite', 'What is missing from the draft.')
...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    hiring = pa.Human(
        ...
        protocol,
        pa.ScriptedHumanIo(
            [pa.says(Company, 'typed agents startup hiring a backend engineer')],
        ),
    )
