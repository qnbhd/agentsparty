...
Asker, Librarian, Index, Auditor = ap.roles('Asker', 'Librarian', 'Index', 'Auditor')

PASSAGES = ap.Text.many(1, 5)
GROUNDED = ap.Text.where(
...
Nothing = ap.Text('Nothing', 'Why the query matched no passage.')
Lookup = ap.Text('Lookup', 'A search query for the index.')
Passages = PASSAGES('Passages', 'Passages that match the query.')
Question = ap.Text('Question', 'What the reader wants to know.')
NoAnswer = ap.Text('No answer', NO_ANSWER)
Answer = GROUNDED('Answer', 'An answer citing the passages.')
Noted = ap.Nothing('Noted', 'Auditor acknowledgement.')
Sound = ap.Nothing('Sound', 'The answer is supported by the passages.')
Unsupported = ap.Text('Unsupported', 'Which claim the passages do not support.')
Blocked = ap.Text('Blocked', 'Why no answer will be attempted.')


@ap.choreography
def retrieval_qa(c: ap.Chor) -> None:
...
def _index_tools(calls: list[str]) -> list[Any]:
    async def lookup(query: str) -> ap.Choice:
        calls.append(query)
        ...
        if hits:
            ...
            return ap.reply(Passages, passages)
        return ap.reply(Nothing, f'no passage matched {query!r}')

    return [ap.tool_for(Lookup, lookup)]


def build(
    *,
    index_calls: list[str] | None = None,
) -> tuple[SessionType, list[ap.Participant], list[str]]:
    client = AsyncOpenAI(
    ...
    asker = ap.Human(
        Asker,
        protocol,
        ap.ScriptedHumanIo([ap.says(Question, 'How long is the battery life?')]),
    )
    participants: list[ap.Participant] = [
        asker,
        librarian,
        auditor,
        ap.Toolbox(Index, protocol, _index_tools(index_calls)),
    ]
...
def main() -> None:
    ...
    runtime = ap.AgentRuntime(protocol, participants, journal=journal)
    ...
    model = ap.OpenAIModel('gpt-5.6-luna', client)
    replay_runtime = ap.AgentRuntime(
        protocol,
        [
            ap.Human(Asker, protocol, ap.ScriptedHumanIo([])),
            ap.Agent(model, Librarian, 'replay', protocol),
            ap.Agent(model, Auditor, 'replay', protocol),
            ap.Toolbox(Index, protocol, _index_tools(replay_calls)),
        ],
