...
Bench, Solver, Judge = pa.roles('Bench', 'Solver', 'Judge')

VERDICT = pa.Integer.between(1, 5)


Problem = pa.Text('Problem', 'The task to solve.')
Solution = pa.Text('Solution', 'The current solution.')
Pass = VERDICT('Pass', 'A score from one to five.')
Accepted = VERDICT('Accepted', 'The score that was awarded.')
Fail = pa.Text('Fail', 'What is wrong with the solution.')
Retrying = pa.Text('Retrying', 'Progress note.')
...
def _bench_decide(view: pa.View) -> pa.Choice:
    offered = {label.name for label in view.offered}
    if 'Problem' in offered:
        return pa.says(Problem, 'return the sum of two integers')
    return pa.Choice(view.offered[0])


def _judge_decide(view: pa.View) -> pa.Choice:
    solutions = [e.payload for e in view.seen if e.label == Label('Solution')]
    latest = str(solutions[-1]) if solutions else ''
    if 'a + b' in latest:
        return pa.says(Pass, 5)
    return pa.says(Fail, 'must use addition, not subtraction')
...
def build() -> tuple[SessionType, list[pa.Participant]]:
    ...
    return protocol, [
        pa.Machine(Bench, protocol, _bench_decide),
        solver,
        pa.Machine(Judge, protocol, _judge_decide),
    ]
