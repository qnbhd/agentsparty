...
Client, Analyst, Strategist, Creative, Director = ap.roles(
    'Client',
    'Analyst',
    'Strategist',
    'Creative',
...
STRATEGY = ap.json_model('Strategy', STRATEGY_SCHEMA, parse_strategy)


COPY = ap.record('Copy', title=str, body=str)
...
Idea = ap.Text('Idea', 'One campaign idea and its audience.')
Approve = ap.Nothing('Approve', APPROVE)
Campaign = COPY('Campaign', CAMPAIGN)
StrategyCase = STRATEGY('Strategy', 'Name, tactics and channels.')
Domain = ap.Text('Domain', 'The customer domain and the project.')
Research = ap.Text('Research', 'What the market looks like.')
Rework = ap.Text('Rework', 'One sharp critique, at most twenty-five words.')


@ap.choreography
def campaign(c: ap.Chor) -> None:
...
def build() -> tuple[SessionType, list[ap.Participant]]:
    ...
    client = ap.Human(
        Client,
        protocol,
        ap.ScriptedHumanIo([ap.says(Domain, 'B2B tooling for agent crews')]),
    )
