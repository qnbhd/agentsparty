...
def _composed() -> tuple[ap.LanguageModel, ap.Metered]:
    ...
    reserve = ap.OpenAIModel('gpt-5.6-luna', client)
    primary = ap.Retrying(ap.Unavailable('the primary endpoint is down'), attempts=2)
    # High budget: live calls bill real tokens; meter still records usage.
    meter = ap.Metered(ap.fallback(primary, reserve), tokens=100_000)
    return ap.traced(meter), meter


def build() -> tuple[SessionType, list[ap.Participant], ap.Metered, ap.MemoryTracer]:
    worker_model, worker_meter = _composed()
    reviewer_model, _reviewer_meter = _composed()
    worker = ap.Agent(worker_model, Worker, 'Do the work in one short draft.', protocol)
    reviewer = ap.Agent(reviewer_model, Reviewer, 'Accept or note one change.', protocol)
    client = ap.Human(
        Client,
        protocol,
        ap.ScriptedHumanIo([ap.says(Task, 'show model composition')]),
    )
...
def main() -> None:
    ...
    spent = ap.Metered(
        ap.ScriptedLanguageModel(['{"alt": {"label": "Draft", "payload": "unused"}}']),
        tokens=0,
    )
    with report.refusing(ap.TokenLimitError, title='a spent meter'):
        asyncio.run(
            spent.complete(
                ap.StructuredRequest(
                    instructions='n/a',
