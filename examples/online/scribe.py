...
Worker, Scribe = ap.roles('Worker', 'Scribe')

Progress = ap.Text('Progress')
CarryOn = ap.Nothing('Carry on')
Digest = ap.Text('Digest')
Finish = ap.Text('Finish')
...
def _scribe_decide(threshold: int = 3):
    ...
    def decide(view: ap.View) -> ap.Choice:
        ...
        if len(progresses) >= threshold * 2:
            ...
            return ap.says(Finish, f'done: {", ".join(units)}')
        if (
            len(progresses) > 0
            ...
            return ap.says(Digest, f'summary of {", ".join(recent)}')
        return ap.says(CarryOn)
...
def build_participants(threshold: int = 3) -> list:
    ...
    worker = ap.Agent(
        ...
        protocol,
        brief=ap.Recent(Worker, keep=4),
    )
    return [
        worker,
        ap.Machine(Scribe, protocol, _scribe_decide(threshold)),
    ]
