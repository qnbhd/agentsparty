...
What you will see (sample run):
  ...
  === conversation ===
  Brand:Product -> Analyst 'agentsparty: multiparty session protocols for agents'
...
Brand, Analyst, Strategist, Copywriter, Photographer, Director = ap.roles(
    'Brand',
    'Analyst',
    'Strategist',
    'Copywriter',
...
COPY = ap.record('Copy', hook=str, body=str)
SHOT = ap.Text.where(
...
Angle = ap.Text('Angle', 'The campaign angle.')
Approved = SHOT('Approved', 'The approved photograph description.')
Product = ap.Text('Product', 'The product page and extra details.')
WRONG = COPY('WRONG', 'Hook and body for the post.')
Findings = ap.Text('Findings', 'Selling points and competitors.')
...
def build() -> tuple[SessionType, list[ap.Participant]]:
    ...
    brand = ap.Human(
        ...
        protocol,
        ap.ScriptedHumanIo([
            ap.says(Product, 'agentsparty: multiparty session protocols for agents')
        ]),
    )
...
def main() -> None:
    ...
    with report.refusing(ap.CompositionError, title='an incompatible component'):
