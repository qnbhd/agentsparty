...
Verifying = pa.Nothing('Verifying', 'Reviewer is still reading files.')
Checked = pa.Nothing('Checked', 'File reading is finished.')
...
def build_cast(root: Path, client_io: Any, model: pa.LanguageModel) -> pa.Cast:
    ...
    return (
        ...
        .play(
            Planner, pa.agent(model, PLANNER_BRIEF, effort='medium', repair=pa.Repair(attempts=2))
        )
        .play(Coder, pa.agent(model, CODER_BRIEF, effort='medium', repair=pa.Repair(attempts=2)))
        .play(Reviewer, pa.agent(model, REVIEWER_BRIEF, repair=pa.Repair(attempts=2)))
...
def _openai_model() -> pa.LanguageModel:
    ...
    return pa.OpenAIModel(MODEL_NAME, client)
...
class Studio(App[None]):
    ...
    def __init__(self, root: Path, model: pa.LanguageModel) -> None:
    ...
    def on_mount(self) -> None:
        ...
        self.query_one('#head', Static).update(
            f'[{MUTED}]#[/] [b {ACCENT}]agentsparty coding agent[/] [{MUTED}]v{pa.__version__}[/]\n'
    ...
    async def _session(self) -> None:
        ...
        tracer = pa.QueueTracer()
        cast = build_cast(self._root, TuiHumanIo(self), pa.traced(self._model))
        session = cast.run(allowance=pa.Allowance(unfoldings=40), tracer=tracer)
        async with pa.watching(session, tracer) as watch:
            try:
    ...
    def _fold(self, signal: Signal) -> None:
        ...
        match signal:
            case pa.StepStarted(sender=sender, offered=offered):
                self.query_one(Pulse).waiting(sender, offered)
            case pa.Delivered(envelope=envelope):
                self._steps += 1
    ...
    def _remark(self, signal: Signal) -> None:
        ...
        match signal:
            ...
            case pa.SessionFinished(messages=messages):
